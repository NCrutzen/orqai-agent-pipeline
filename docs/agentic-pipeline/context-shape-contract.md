...
| `entity_brand` | yes | no | Today: derived from `debtor.labeling_settings.brand_id` + `debtor.labeling_settings.nxt_database` per source mailbox. Phase 68 unifies these into the `swarms.entity_brand` registry as the single source of truth; today's per-mailbox columns become a denormalised read-model. **Phase 69 expanded the registry shape from a flat string array to jsonb-of-objects with per-brand metadata** (`code`, `display_name`, `register_language`, `register_dialect`, `signoff_phrase`, `formal_address`, `nxt_database_alias`, `icontroller_company`) -- see [`supabase/migrations/20260505a_entity_brand_expansion.sql`](../../supabase/migrations/20260505a_entity_brand_expansion.sql). The wire-shape on `PipelineStageContext` stays a `string` (the brand `code`); Stage 4 handlers resolve the full metadata object via `loadBrandRegister(swarm_type, brand_code)` from `web/lib/swarms/brand-register.ts`. | Drives all downstream routing: prompts, categories, tool allowlists. The TypeScript type is `string` on purpose -- the registry, not the type system, is the source of truth. See [Forward References](#forward-references). |
...
- ~~`swarms.entity_brand` unified registry -- Phase 68 (SWRM-01).~~ **DONE in Phase 68** (registry seeded as text[] then expanded to jsonb-of-strings) **+ Phase 69 (CANO-02)** (expanded to jsonb-of-objects with per-brand metadata; see `20260505a_entity_brand_expansion.sql`).
