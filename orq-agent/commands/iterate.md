...
**Command format:** `/orq-agent:iterate [agent-key]` where `agent-key` is optional.

- If `agent-key` is provided: filter iteration to that single agent
- If no `agent-key`: iterate on all failing agents in the swarm
