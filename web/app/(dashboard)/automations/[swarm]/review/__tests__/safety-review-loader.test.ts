...
interface Scenario {
  // Rows the safety query returns.
  safetyRows: Array<{ id: string; topic: string; result: Record<string, unknown> }>;
  // Outlier RPC payload.
  ...
  // Generic predicted-list query (used by the non-safety branch — regression test).
  predictedRows: Array<{ id: string; topic: string | null }>;
}
...
const adminClientMock = {
  from: vi.fn((table: string) => {
    ...
    if (table === "automation_runs") {
      // The loader uses the same builder for both predicted-list and safety-list.
      // We resolve with whichever rows the test scenario configured for the active branch.
...
beforeEach(() => {
  ...
  scenario = {
    ...
    countsRows: [],
    candidatesRows: [],
    predictedRows: [{ id: "ar-pred-1", topic: "credit_question" }],
  };
});

describe("loadPageData — ?tab=safety branch (Phase 64-05)", () => {
  it("filters automation_runs on topic='safety_review' when params.tab==='safety'", async () => {
    currentTab = "safety";
    ...
    expect(eqCols).toContain("topic=safety_review");
    expect(eqCols).toContain("status=predicted");
    expect(eqCols).toContain("swarm_type=debtor-email");
  });

  it("calls automation_runs_with_outlier RPC and merges is_cost_outlier into rows", async () => {
    currentTab = "safety";
    const params: PageSearchParams = { tab: "safety" };
    // @ts-expect-error — admin shape is structurally compatible for test.
    ...
    const r1 = data.rows.find((r) => r.id === "ar-safety-1") as unknown as { is_cost_outlier: boolean };
    const r2 = data.rows.find((r) => r.id === "ar-safety-2") as unknown as { is_cost_outlier: boolean };
    expect(r1.is_cost_outlier).toBe(true);
    expect(r2.is_cost_outlier).toBe(false);
  });

  it("preserves stage-0 result fields needed by the detail pane", async () => {
    currentTab = "safety";
    const params: PageSearchParams = { tab: "safety" };
    // @ts-expect-error — admin shape is structurally compatible for test.
    const data = await loadPageData(params, adminClientMock, "debtor-email");

    const r = data.rows.find((row) => row.id === "ar-safety-1");
  ...
  it("regression: when params.tab is not 'safety', the loader does NOT filter on topic='safety_review'", async () => {
    currentTab = undefined;
    ...
    expect(lastListBuilder).not.toBeNull();
    const eqCols = lastListBuilder!._eqCalls.map((c) => `${c.col}=${c.val}`);
    expect(eqCols).not.toContain("topic=safety_review");
    // Existing branch still filters by status+swarm_type.
    expect(eqCols).toContain("status=predicted");
    expect(eqCols).toContain("swarm_type=debtor-email");
  });
});
