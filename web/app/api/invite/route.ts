...
const inviteSchema = z.object({
  ...
  supabaseUserId: z.uuid().optional(),
});
...
export async function POST(request: Request) {
  try {
    ...
    const { email, projectId, supabaseUserId } = parsed.data;
    ...
    if (supabaseUserId) {
      // Path A: AD user already in Supabase -- add directly to project
      const { error: memberError } = await supabaseAdmin
        .from("project_members")
        .upsert(
          { project_id: projectId, user_id: supabaseUserId },
      ...
      // Path B: External user -- invite via email
