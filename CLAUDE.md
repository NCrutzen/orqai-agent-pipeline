...
- Voor cross-endpoint JSON-enforcement (Studio test + production): naast `model.parameters.response_format` óók een **`json_schema` tool resource** in `settings.tools`. MCP exposeert geen tool CRUD; via Studio dashboard aanmaken, of via `POST /v2/tools` met een API key die de juiste catalog-write scope heeft (zonder die scope krijg je 403 — controleer scopes in Orq Studio → Settings → API Keys).
