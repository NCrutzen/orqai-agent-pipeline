...
## Phase 76 Validation Results — Plan 76-08 (orchestrator-direct live-DB pass — 2026-05-07)

Executed by the orchestrator via `mcp__supabase__execute_sql` against project `mvqjhlxfvtqqubqgdvhz`. User explicitly authorized live writes through AskUserQuestion ("Approve live-DB writes now"). All three triggers validated end-to-end and all synthetic rows cleaned up.

### Trigger validation — Strategy A (synthetic INSERT into `automation_runs`)

| Trigger | INSERTed id | reason | status | topic | Result |
|---|---|---|---|---|---|
| `no_handler` | `8423ae1d-6401-4920-a53a-69eb1d1cd5cd` | `no_handler` | `pending` | `general_inquiry` | ✓ |
| `low_confidence` | `70122fc0-3938-4dc8-b652-909bc59edaf6` | `low_confidence` | `pending` | `invoice_copy_request` | ✓ |
| `handler_error` | `ae968143-56cb-4839-85c6-c8b9f954cb44` | `handler_error` | `pending` | `invoice_copy_request` | ✓ |

### Loader-shape verification (mirrors `loadKanbanRows`)

Filter: `swarm_type='debtor-email' AND status='pending' AND result->>'kanban_reason' IS NOT NULL`. Per-trigger GROUP BY confirmed `handler_error=1, low_confidence=1, no_handler=1` — exactly what the loader would surface to Stage 3 / Stage 4 panes.

### Operator-action exercise — Close

`UPDATE automation_runs SET status='completed', completed_at=now() WHERE id='8423ae1d-…' AND swarm_type='debtor-email' AND status='pending'` returned 1 row with `status='completed'` and `has_completed_at=true`. This validates the DB-side contract of `_actions/close.ts:35` (the `swarm_type` + `status='pending'` guards prevent stale double-close + cross-swarm IDOR).

Replay and Reclassify-as-noise contracts are covered by the in-scope vitest suite (Plan 76-05 SUMMARY: 27/27 GREEN); not re-exercised against live DB to keep the synthetic write surface minimal.

### Cleanup

`DELETE FROM automation_runs WHERE result->>'test_marker' LIKE 'PHASE76-VALIDATION-2026-05-07-%'` deleted 3 rows (all three test markers returned). Sanity SELECT confirmed `remaining=0`. No `pipeline_events` writes were generated by Strategy A (no operator action besides Close, and Close does not write pipeline_events) so no FK cleanup was needed.

### Status

✓ Tasks 1+2+3 of Plan 76-08 complete. All three Kanban trigger conditions validated against live Supabase. No production data touched; all synthetic rows removed.
