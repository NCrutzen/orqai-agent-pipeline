...
| 008 | detail-pane-stage-scope | Future stages + Approve semantics — full surface stack (header, body, audit per stage, feedback panel, read-back). | **A + context-morph** (future-stages collapsed; bottom button morphs Approve↔Submit override on dirty state) | phase-82.5, detail-pane, stage-scope, approve-semantics, surface-stack |
| 009 | row-strip-review-indicators | What chip/treatment shows verdict status on rows without clashing with stage badges? | **A — 8px dot before stage badge** | phase-82.5, bulk-review, row-strip, verdict-indicator |
| 010 | override-note-coupling-cue | When the override picker opens, how do we make it obvious the textarea note attaches? | **C — Inline microcopy** | phase-82.5, override, coupling, detail-pane |
| 011 | save-vs-confirm-clarity | What treatment makes "Save (revisit)" vs "Confirm (done)" immediately differentiable? | **C — Iconography + microcopy** | phase-82.5, feedback, save, confirm, microcopy |
