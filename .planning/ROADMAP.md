...
| **v8.1** | Validation + Visibility -- body ingestion fix, Stage 1 noise rules, Stage 3 prompt v3 + open-set discovery, retro-classification baseline, review-surface cleanup, stage-named Inngest functions, LLM 2nd-pass auto-action promotion track (Phases 83-89) | **Defined** |
| **V9.0** | Promotion Recommender + Learning Inbox -- T2 synthesis on captured `email_feedback`, Learning Inbox tab, data-driven resolver steps, immediate-apply new intents, granular dry-run gating | **Defined** |
| **V10.0** | Sales-email second-operator validation -- Sugar-side Stage 3 intent agent, second operator using V9.0 Learning Inbox, multi-operator scaffolding (`operator_id`, scoped filtering, vocab reconciliation) | **Defined** |
...
- [x] **Phase 83: Body ingestion — capture full thread on forwards and replies** — Stage 3 misclassifies forwards and replies because Outlook ingest stores only Graph's `uniqueBody` (the new bit), dropping the quoted debtor original. Evidence in `email_pipeline.emails` 2026-05-19: `body_text=∅, body_html=∅, raw_json=[]` on `FW:` rows; Elger's `Re: Re:` chains store only his reply, never the originating debtor message. Fix at `web/lib/outlook/client.ts:207` (prefer `body` over `uniqueBody`), persist `body_html` + `raw_json`, add `body_full_text` / `body_unique_text` columns, conversation-aware fallback via `conversationId`, 90-day backfill. CONTEXT exists; needs `/gsd-discuss-phase 83`.
  ...
  **Plans:** 8 plans (wave layout: 1→01, 2→02, 3→03, 4→04, 5→05, 6→06+06b, 7→07)
  ...
  - [x] 83-06-PLAN.md — Stage 3 coordinator reads body_full_text; emits wrapped XML assembled_input with D-08 truncation + D-09 cap + telemetry (D-06 Stage 3, D-08, D-09, D-10)
  - [x] 83-06b-PLAN.md — Stage 1 regex screen worker reads body_full_text with body_text fallback (D-06 Stage 1, D-10) — parallel sibling of 83-06 in wave 6
  - [x] 83-07-PLAN.md — Read-only verification harness + operator §1/§2/§3/§5 sign-off checkpoint
...
- [ ] **Phase 88: Review-surface cleanup — operator mental-model fixes on the unified `_shell/`** — Three operator-confusion items surfaced during the 2026-05-18/19 debtor-person UAT, all on the same unified stage shell, all pure frontend. (D-01) Override + note flow consolidation on Stages 0/2/3 — collapse the three-button surface (`Save note` / `Confirm` / `Submit override`) + two "save together" hints into a single footer-driven submit path per Phase 82.7.3 H-01 trajectory; (D-02) Stage 1 "Needs review" chip semantics — rename today's decision-based chip to "Unclassified" and add a verdict-based "Pending my review" chip backed by `own_latest_verdict IS NULL`, applied consistently to Stage 0/1/2 list-loaders; (D-03) Stage 4 layout parity — replace Phase 82.8-05 collapsible sections with the Stage 1/2/3 chip-strip pattern (Handler error / Needs review / Auto-archived chips), and fix the detail-pane width regression (measured Stage 4 pane narrower than 540px baseline despite identical grid template — likely sticky-wrapper collapse from Phase 82.8-11). Pure frontend; no pipeline / server-action / DB changes. **Source todos:** `2026-05-18-override-note-flow-unclear.md`, `2026-05-18-stage-1-needs-review-semantics.md`, `2026-05-19-stage-4-layout-parity-and-detail-pane-width.md`. Needs `/gsd-discuss-phase 88`. **Depends on:** Phase 82.8 (Stage 4 sections to replace); independent of 83-87.
- [ ] **Phase 88.1: Stage-named Inngest functions + Stage 2 telemetry alignment** — Rename `classifier-label-resolver` → `stage-2-customer-resolver`, `debtor-email-icontroller-tagger` → `stage-2-icontroller-label-applier`, `debtor-email-icontroller-cleanup-worker` → `stage-1-icontroller-noise-cleanup` for naming clarity (operator confused 2026-05-19: "why are there a tagger and a labeler?"). Lock the tagger as Stage 2 in artifacts: emit `pipeline_events.stage=2` on completion, mount tagger screenshots under `effectiveStageAudit[2]` (cleanup-worker stays at `[1]`), document the resolve→apply Stage 2 split in `docs/agentic-pipeline/stage-2-entity.md`. Touches Inngest function IDs (deploy-risky — treat as new functions), event names (`debtor-email/label-resolve.requested` → `debtor-email/stage-2.customer-resolve.requested`), 4 RFC docs, CLAUDE.md. **Higher blast radius than Phase 88 — own phase, own deploy window.** Could slip to V9 if v8.1 calendar tightens. **Source todo:** `2026-05-19-rename-label-resolver-and-tagger-plus-stage-alignment.md`. Needs `/gsd-discuss-phase 88.1`. **Depends on:** Phase 82.8 (screenshot strip mount-point logic).
- [ ] **Phase 89: Stage 1 LLM 2nd-pass auto-action promotion track** — Today Stage 1 LLM verdicts (`stage-1-category-classifier`, Phase 74) emit `matchedRule="no_match"` because the LLM bypasses the regex path. The `classifier-screen-worker` auto-action gate (`isWhitelistMatch = whitelistSet.has(matchedRule)`) therefore evaluates to `false` for every LLM verdict, regardless of `category_key` or `confidence` — every LLM-classified noise email lands in bulk-review instead of auto-archive. Operationally observed 2026-05-19: of 9 Stage 1 noise verdicts in one day, 3 were LLM 2nd-pass and all 3 went to bulk-review; combined with 4 demoted-regex-rule rows (now fixed by `d0ff6ab`), only 1 of 9 noise emails actually auto-archived. **Fix per option-1 ("LLM-promotion track"):** extend the existing Wilson-CI promotion model to LLM verdicts. Mint synthetic `rule_key` strings of the form `llm:{category_key}:{confidence}` (e.g. `llm:auto_reply:high`, `llm:payment_admittance:high`) in `classifier_rules` with the same `candidate → promoted → demoted` lifecycle. Stage 1 worker sets `matchedRule = "llm:" + categoryKey + ":" + confidence` on LLM-derived verdicts so the existing whitelist check works without code changes downstream. Telemetry view (`classifier_rule_telemetry`) is widened to aggregate `email_feedback.verdict` per LLM rule_key from `agent_runs`. Promotion gate stays at `n≥30 AND ci_lo≥0.92`; demotion gate respects the 2026-05-20 `DEMOTE_N_MIN=30` floor. Initial seed: insert `candidate` rows for all `(noise_category × {high})` combinations from `swarm_noise_categories` — `low`/`medium` confidence stays in bulk-review by design (gate-narrow first, expand only when the high-confidence track proves out). Cross-swarm by default; no debtor-specific branches. **Acceptance:** (a) `classifier_rules` carries `llm:*:high` rows; (b) Stage 1 worker `matchedRule` for LLM verdicts uses the new format; (c) a Wilson-CI shadow-mode run on 30 days of corpus data identifies at least 1 promotable LLM rule_key; (d) post-promotion, an LLM verdict with category+confidence matching a promoted rule_key produces an auto-archive cleanup row (verified by spot-checking `automation_runs` for `triggered_by='stage-1-worker', result.stage='categorize+archive'`). **Source debug session:** `.planning/debug/stage-2-customer-mapping-stuck.md` (item C of the post-fix decision). Needs `/gsd-discuss-phase 89`. **Depends on:** none (independent of 83-88); benefits from Phase 84 landing first (more regex-rule baseline data), but does not block.
- [ ] `/gsd-audit-milestone v8.1` — formal closure after Phases 87 + 88 (+ 88.1, 89 if not slipped) pass.
...
**Wave 1**
Plans:
- [x] 82.9-01: Zapier `nxt.candidate_details` schema extension (contact_person + recent_invoices) + Zod + registry UPDATE

**Wave 2** *(blocked on Wave 1 completion)*
- [x] 82.9-02: Resolver widening + writer extension (ResolveResult discriminated `inputs`, classifier-label-resolver decision_details payload)
